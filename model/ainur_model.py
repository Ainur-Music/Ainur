...
class Ainur(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        train_dataset , *_ = random_split(self.dataset, [0.99, 0.005, 0.005], torch.Generator().manual_seed(42))
        ...
        with torch.no_grad():
            ...
            if not os.path.exists(tmp_dir):
                os.mkdir(tmp_dir)
            torchaudio.save(os.path.join(tmp_dir, "original.wav"), audio[0].detach(), 48_000) 
            torchaudio.save(os.path.join(tmp_dir, "sample_lyrics.wav"), evaluation_lyrics[0].detach(), 48_000)
            torchaudio.save(os.path.join(tmp_dir, "sample_audio.wav"), evaluation_audio[0].detach(), 48_000)
            torchaudio.save(os.path.join(tmp_dir, "sample_noclip.wav"), evaluation_noclip[0].detach(), 48_000)
    ...
    def test_step(self, batch, batch_idx):
        ...
        train_dataset , *_ = random_split(self.dataset, [0.99, 0.005, 0.005], torch.Generator().manual_seed(42))
        background, _ = random_split(train_dataset, [0.1, 0.7], torch.Generator().manual_seed(42))
        background = map(lambda item : item[0], background)
        ...
        with torch.no_grad():
            ...
            if not os.path.exists(tmp_dir):
                os.mkdir(tmp_dir)
            torchaudio.save(os.path.join(tmp_dir, "original.wav"), audio[0].detach(), 48_000) 
            torchaudio.save(os.path.join(tmp_dir, "sample_lyrics_test.wav"), evaluation_lyrics[0].detach(), 48_000)
            torchaudio.save(os.path.join(tmp_dir, "sample_audio_test.wav"), evaluation_audio[0].detach(), 48_000)
            torchaudio.save(os.path.join(tmp_dir, "sample_noclip_test.wav"), evaluation_noclip[0].detach(), 48_000)
            self.logger.experiment.log_audio(os.path.join(tmp_dir, "original.wav"))
    ...
    def test_dataloader(self):
        *_, test_dataset = random_split(self.dataset, [0.99, 0.005, 0.005], torch.Generator().manual_seed(42))
    ...
    def val_dataloader(self):
        _, val_dataset, _ = random_split(self.dataset, [0.99, 0.005, 0.005], torch.Generator().manual_seed(42))
    ...
    def sample(self, lyrics=None, audio=None, n_samples=1, **kwargs):
        ...
        if lyrics is not None:
            ...
            latent = torch.zeros(n_samples, 1, 2**self.latent_factor) # no clip conditioning
